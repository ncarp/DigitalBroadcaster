// BlocksFrame.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CBlocksFrame, CMDIChildWnd)

CBlocksFrame::CBlocksFrame()
{
}
...
BEGIN_MESSAGE_MAP(CBlocksFrame, CMDIChildWnd)
	//{{AFX_MSG_MAP(CBlocksFrame)
	ON_WM_CREATE()
...
/////////////////////////////////////////////////////////////////////////////
// CBlocksFrame message handlers

int CBlocksFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	TBBUTTON	buttons[10];
...
BOOL CBlocksFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	CRadioStation dl;
	...
	m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CBlocksLeft), CSize(400, 100), pContext);
	m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CHtmlView),CSize(100,100), pContext);

	CBlocksLeft *pView = (CBlocksLeft *)m_wndSplitter.GetPane(0,0);
...
void CBlocksFrame::OnNew()
{
	CBlocksLeft *pView = (CBlocksLeft *)m_wndSplitter.GetPane(0,0);
	pView->OnNew();
}

void CBlocksFrame::OnEdit()
{
	CBlocksLeft *pView = (CBlocksLeft *)m_wndSplitter.GetPane(0,0);
	pView->OnDoubleClick();
}

void CBlocksFrame::OnCut()
{
	CBlocksLeft *pView = (CBlocksLeft *)m_wndSplitter.GetPane(0,0);
	pView->OnCut();
}
