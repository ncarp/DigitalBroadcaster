// MusicCategoryWnd.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CMusicCategoryWnd, CFrameWnd)

CMusicCategoryWnd::CMusicCategoryWnd()
{
}
...
BEGIN_MESSAGE_MAP(CMusicCategoryWnd, CFrameWnd)
	//{{AFX_MSG_MAP(CMusicCategoryWnd)
	ON_WM_SYSCOMMAND()
	ON_WM_SYSKEYUP()
	ON_WM_KEYUP()
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CMusicCategoryWnd message handlers

void CMusicCategoryWnd::OnSysCommand(UINT nID, LPARAM lParam) 
{
	// Do Nothing
}

void CMusicCategoryWnd::OnSysKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if( !CheckKey(nChar) )
		CFrameWnd::OnSysKeyUp(nChar, nRepCnt, nFlags);
}

void CMusicCategoryWnd::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if( !CheckKey(nChar) )
		CFrameWnd::OnKeyUp(nChar, nRepCnt, nFlags);
}

void CMusicCategoryWnd::OnPaint() 
{
...
int CMusicCategoryWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CMusicCategoryWnd::OnRefresh()
{
...
void CMusicCategoryWnd::OnSelChange()
{
...
UINT CMusicCategoryWnd::ThreadRefreshList(LPVOID lpv)
{
	...
	CString					strTemp;
	CMusicCategoryWnd*		pWnd=(CMusicCategoryWnd*)lpv;
	...
	long	lCategoryID=pWnd->m_pSelection->GetItemData(iCurSel);

	rs.m_strFilter.Format("TrackID in (Select ID from Broadcaster.dbo.MusicTracks where CategoryID = '%d')",lCategoryID);
...
void CMusicCategoryWnd::OnInfo()
{
	...
	{
		strURL.Format("%s\\Artist.asp?ID=%d",g_strBaseURL);
		g_wndMusicInfo->SetView(strURL);
	}
}
