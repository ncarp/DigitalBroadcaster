// NewsLineUpFrame.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CNewsLineUpFrame, CMDIChildWnd)

CNewsLineUpFrame::CNewsLineUpFrame()
{
}
...
BEGIN_MESSAGE_MAP(CNewsLineUpFrame, CMDIChildWnd)
	//{{AFX_MSG_MAP(CNewsLineUpFrame)
	ON_WM_CREATE()
...
/////////////////////////////////////////////////////////////////////////////
// CNewsLineUpFrame message handlers

int CNewsLineUpFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	TBBUTTON	buttons[10];
...
BOOL CNewsLineUpFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	CRadioStation dl;
	dl.DoModal();

	m_wndSplitter.CreateStatic(this,1,2);
	m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CNewsLineUpLeftView), CSize(400, 100), pContext);
	m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CHtmlView),CSize(100,100), pContext);

	CNewsLineUpLeftView *pView = (CNewsLineUpLeftView *)m_wndSplitter.GetPane(0,0);
...
void CNewsLineUpFrame::OnNew()
{
	CNewsLineUpLeftView *pView = (CNewsLineUpLeftView *)m_wndSplitter.GetPane(0,0);
	pView->OnNew();
}

void CNewsLineUpFrame::OnEdit()
{
	CNewsLineUpLeftView *pView = (CNewsLineUpLeftView *)m_wndSplitter.GetPane(0,0);
	pView->OnDoubleClick();
}

void CNewsLineUpFrame::OnCut()
{
	CNewsLineUpLeftView *pView = (CNewsLineUpLeftView *)m_wndSplitter.GetPane(0,0);
	pView->OnCut();
}
